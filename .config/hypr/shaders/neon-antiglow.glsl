...
vec4 chroma_aberration(sampler2D texture, vec2 uv, float chroma_distance, float speed, float intensity) {
    float dist = chroma_distance /* * (1.0 + 0.2 * sin(time * speed)) */ ;
...
vec4 apply_crt(vec4 baseColor, vec2 uv) {
    ...
    const float scanline_speed = 2.0; // Velocidad scanlines
    ...
    vec4 finalColor = baseColor * flicker;
...
void main() {
    ...
    float speed = 2.0;
    float chroma_inst = 1.4;
    vec4 chroma = chroma_aberration(tex, v_texcoord, chroma_distance, speed, chroma_inst);
